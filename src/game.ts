...
function initialLocationLastVisited(): Record<LocationId, number> {
  return Object.fromEntries(
    locations.map((location) => [location.id, location.id === "mau" ? 0 : -12]),
  ...
  return [
    {
      id: "fy1",
      name: "FY1",
      role: "Keen, overloaded, good for contained jobs",
      busyUntil: 0,
      strengths: ["routine jobs", "simple prescribing", "low-risk reviews"],
      trust: 60,
      fatigue: 18,
      recentDelegations: 0,
    },
    {
      id: "trusted_fy2",
      name: "Trusted FY2",
      role: "Solid with sick-ish patients if briefed well",
      busyUntil: 0,
      strengths: ["urgent reviews", "handover jobs", "clear escalation"],
      trust: 68,
      fatigue: 22,
      recentDelegations: 0,
    },
    {
      id: "locum_no_login",
      name: "Locum without login",
      role: "Clinically useful, digitally cursed",
      busyUntil: 0,
      strengths: ["bedside assessment", "practical ward help"],
      trust: 42,
      fatigue: 28,
      recentDelegations: 0,
    },
    {
      id: "bed_manager",
      name: "Bed manager",
      role: "Flow, beds, transport, and applied diplomacy",
      busyUntil: 0,
      strengths: ["flow", "systems", "logistics"],
      trust: 58,
      fatigue: 20,
      recentDelegations: 0,
    },
...
function initialResources(): ResourceItem[] {
  return [
    ...
    },
    {
      ...
      label: "Guideline app",
      charges: 2,
      description: "Clinical confidence during an encounter",
      usableWhen: "encounter",
    },
    {
      id: "abg_kit",
      label: "ABG kit",
      charges: 1,
      description: "Respiratory/metabolic shortcut",
      usableWhen: "encounter",
    },
    {
      id: "cannula_kit",
      ...
      charges: 1,
      description:
        "Use when juggling tasks: +stamina, +patient safety, +reputation, 1m",
      usableWhen: "task",
    },
    {
      id: "consultant_advice",
      label: "Consultant advice token",
      charges: 1,
      description: "Safer escalation on a hard case",
      usableWhen: "encounter",
    },
    {
      id: "radiology_persuasion",
      label: "Radiology persuasion token",
      charges: 1,
      description:
        "Use while tasks are active: +reputation, +patient safety, +score, 2m",
      usableWhen: "task",
    },
  ];
}
...
function makeTask(
  ...
  const intelLevel =
    template.vague || template.regSense || template.category === "ambiguous"
      ? 0
      : template.trueUrgency === "nonsense"
        ? 2
        : 1;
  return {
    id,
...
function escalationTargetFor(
  task: Pick<
    ...
    "locationId" | "category" | "source" | "trueUrgency" | "encounterId"
  >,
): EscalationTarget {
  if (task.locationId === "radiology") return "radiology";
  if (
    task.locationId === "icu" ||
    task.trueUrgency === "critical" ||
    task.category === "emergency"
  )
    return "ICU";
  if (
    task.source === "system" ||
    ["mau", "corridor", "lifts", "estates"].includes(task.locationId)
  )
    return "site";
  if (
    task.encounterId ||
    ["cardiology", "respiratory", "elderly", "surgical", "pharmacy"].includes(
      task.locationId,
    )
  )
    return "specialty";
...
function isBirdTask(task: Pick<ActiveTask, "templateId" | "message">): boolean {
  return /\b(bird|pigeon|duck|ducklings)\b/i.test(
    `${task.templateId} ${task.message}`,
  );
}

function isBirdEncounter(encounter: Encounter): boolean {
  return /\b(bird|pigeon|duck|ducklings|winged)\b/i.test(
    `${encounter.id} ${encounter.title} ${encounter.vignette}`,
  );
}

function sightBirdIfNeeded<T extends { birdStatus: GameState["birdStatus"] }>(
  state: T,
): T {
  return state.birdStatus === "unseen"
    ? { ...state, birdStatus: "sighted" }
    : state;
}

function resolveBirdTaskStatus(
  state: GameState,
  task: ActiveTask,
  outcome: "contained" | "loose" | "sighted",
): GameState {
  if (!isBirdTask(task)) return state;
  if (outcome === "contained") return { ...state, birdStatus: "contained" };
  if (outcome === "loose" && state.birdStatus !== "contained")
    return { ...state, birdStatus: "loose" };
  return sightBirdIfNeeded(state);
}

function resolveBirdEncounterStatus(
  state: GameState,
  encounter: Encounter,
  choice: EncounterChoice,
): GameState {
  if (!isBirdEncounter(encounter)) return state;
  if (choice.id === "best" && !choice.unsafe)
    return { ...state, birdStatus: "contained" };
  if (choice.unsafe && state.birdStatus !== "contained")
    return { ...state, birdStatus: "loose" };
  return sightBirdIfNeeded(state);
}
...
function rebalanceDeterioration(template: TaskTemplate): number {
  ...
  if (template.trueUrgency === "high")
    return clamp(Math.max(original, original + 15), 25, 60);
  if (template.trueUrgency === "medium")
    return clamp(Math.max(original + 35, 60), 60, 120);
...
function pickStartingTemplate(
  pool: TaskTemplate[],
  seed: number,
  usedIds: Set<string>,
): [TaskTemplate | undefined, number] {
  return chooseWeighted(
    pool.filter((template) => !usedIds.has(template.id)),
    seed,
  );
}
...
function startingTasks(state: GameState): [ActiveTask[], number] {
  ...
  const earlyTemplates = taskTemplates.filter((template) =>
    phaseAllowed(template, phase),
  );
  const pools = [
    earlyTemplates.filter(
      (template) =>
        ["critical", "high"].includes(template.trueUrgency) &&
        template.source === "pager",
    ),
    earlyTemplates.filter(
      (template) =>
        template.regSense ||
    ...
    earlyTemplates.filter((template) =>
      ["routine", "inappropriate", "absurd"].includes(template.category),
    ),
  ];
  const usedIds = new Set<string>();
  let seed = state.rngSeed;
  const selected = pools.flatMap((pool) => {
    const [template, nextSeed] = pickStartingTemplate(
      pool.length ? pool : earlyTemplates,
      seed,
      usedIds,
    );
    seed = nextSeed;
...
export function initialGameState(seed = 92821): GameState {
  const base: GameState = {
    ...
    log: [
      {
        minute: 0,
        text: "21:00 handover complete. You are the medical registrar. The bleep has opinions.",
        tone: "neutral",
      },
    ],
    ended: false,
...
export function applyConsequence(
  ...
  return checkEnding({
    ...
    patientSafety: clamp(
      state.patientSafety + (consequence.patientSafety ?? 0),
    ),
    pagerBacklog: Math.max(
      0,
      state.pagerBacklog + (consequence.pagerBacklog ?? 0),
    ),
    clinicalConfidence: clamp(
      state.clinicalConfidence + (consequence.clinicalConfidence ?? 0),
    ),
    caffeine: clamp(state.caffeine + (consequence.caffeine ?? 0)),
    score: state.score + (consequence.score ?? 0),
    handoverQuality: clamp(
      state.handoverQuality + (consequence.handoverQuality ?? 0),
    ),
    emergenciesHandled:
      state.emergenciesHandled + (consequence.emergenciesHandled ?? 0),
    patientsStabilised:
      state.patientsStabilised + (consequence.patientsStabilised ?? 0),
    dangerousDelays: state.dangerousDelays + (consequence.dangerousDelays ?? 0),
    inappropriateAvoided:
      state.inappropriateAvoided + (consequence.inappropriateAvoided ?? 0),
    breaksTaken: state.breaksTaken + (consequence.breaksTaken ?? 0),
    datix: state.datix + (consequence.datix ?? 0),
    consultantEscalations:
      state.consultantEscalations + (consequence.consultantEscalations ?? 0),
...
export function dismissDatixAlert(state: GameState): GameState {
  const next = addLog(
    { ...state, datixAlert: undefined, minute: state.minute + 3 },
    "Datix form submitted. That's 3 minutes you'll never get back.",
    "bad",
  );
...
export function addLog(
  state: GameState,
  text: string,
  tone: ShiftLogEntry["tone"] = "neutral",
): GameState {
  return {
    ...state,
...
function remember(
  state: GameState,
  updates: Partial<HandoverMemory>,
): GameState {
  return {
    ...state,
    handoverMemory: {
      notableRisks: uniqueItems([
        ...state.handoverMemory.notableRisks,
        ...(updates.notableRisks ?? []),
      ]).slice(-12),
      unresolvedRisks: uniqueItems([
        ...state.handoverMemory.unresolvedRisks,
        ...(updates.unresolvedRisks ?? []),
      ]).slice(-12),
      clarifiedRisks: uniqueItems([
        ...state.handoverMemory.clarifiedRisks,
        ...(updates.clarifiedRisks ?? []),
      ]).slice(-12),
      escalations: uniqueItems([
        ...state.handoverMemory.escalations,
        ...(updates.escalations ?? []),
      ]).slice(-12),
      deteriorations: uniqueItems([
        ...state.handoverMemory.deteriorations,
        ...(updates.deteriorations ?? []),
      ]).slice(-12),
      wardHotSpots: uniqueItems([
        ...state.handoverMemory.wardHotSpots,
        ...(updates.wardHotSpots ?? []),
      ]).slice(-8),
      delegatedJobs: uniqueItems([
        ...state.handoverMemory.delegatedJobs,
        ...(updates.delegatedJobs ?? []),
      ]).slice(-12),
      markedTasks: uniqueItems([
        ...state.handoverMemory.markedTasks,
        ...(updates.markedTasks ?? []),
      ]).slice(-12),
      resolvedEncounters: uniqueItems([
        ...state.handoverMemory.resolvedEncounters,
        ...(updates.resolvedEncounters ?? []),
      ]).slice(-12),
    },
  };
}

function removeRememberedUnresolved(
  state: GameState,
  task: ActiveTask,
): GameState {
  return {
    ...
    handoverMemory: {
      ...
      unresolvedRisks: state.handoverMemory.unresolvedRisks.filter(
        (item) => item !== task.message,
      ),
    },
  };
}

function alterWardMomentum(
  state: GameState,
  locationId: LocationId,
  pressureDelta: number,
  tags: WardMomentumState["tags"] = [],
  removeTags: WardMomentumState["tags"] = [],
): GameState {
  const current = state.wardMomentum[locationId];
  const nextTags = uniqueItems([
    ...current.tags.filter((tag) => !removeTags.includes(tag)),
    ...tags,
  ]);
  return {
    ...state,
...
function momentumTagsForTask(task: ActiveTask): WardMomentumState["tags"] {
  ...
  if (task.regSense || task.vague || task.category === "ambiguous")
    return ["quietlyUnsafe"];
...
export function handoverMemoryScore(state: GameState): number {
  ...
  const markedLiveTasks = state.activeTasks.filter(
    (task) => task.markedForHandover,
  ).length;
  return clamp(
...
export function handoverDebrief(state: GameState): string[] {
  const memory = state.handoverMemory;
  const strongest =
    memory.escalations[0] ??
    memory.clarifiedRisks[0] ??
    memory.resolvedEncounters[0] ??
    "You kept enough structure in the noise for the day team to start safely.";
  const unresolved = state.activeTasks.find((task) =>
    ["critical", "high", "medium"].includes(task.trueUrgency),
  );
  const delegation = state.team.reduce(
    (best, member) =>
      member.recentDelegations > best.recentDelegations ? member : best,
    state.team[0],
  );
  const hotWard = locations.reduce((best, location) => {
    const bestPressure =
      state.wardMomentum[best.id].pressure + state.wardAcuity[best.id].level;
    const pressure =
      state.wardMomentum[location.id].pressure +
      state.wardAcuity[location.id].level;
    return pressure > bestPressure ? location : best;
  }, locations[0]);
  return [
    `Strongest handover point: ${strongest}`,
    unresolved
      ? `Unresolved risk: ${unresolved.markedForHandover ? "flagged" : "not clearly flagged"} - ${unresolved.message}`
      : "Unresolved risk: no live clinical issue dominated the desk at 09:00.",
...
function syncLegacyPagerIds(state: GameState): GameState {
  return {
    ...state,
    activePagerIds: state.activeTasks.map((task) => task.id),
    pagerBacklog: state.activeTasks.filter((task) => task.source !== "treat")
      .length,
  };
}
  ...
  return {
    ...
    team: state.team.map((member) =>
      member.busyUntil <= state.minute
        ? { ...member, busyUntil: 0, fatigue: clamp(member.fatigue - 2) }
        : member,
    ),
  };
}
...
export function delegationDuration(
  task: ActiveTask,
  memberId: TeamMemberId,
): number {
  ...
  const base =
    task.trueUrgency === "critical"
      ? 30
      : task.trueUrgency === "high"
        ? 28
        : task.trueUrgency === "medium"
          ? 24
          : 20;
  const adjusted =
    memberId === "locum_no_login" && task.encounterId
      ? base - 3
      : memberId === "locum_no_login" &&
          (task.source === "system" ||
            ["routine", "inappropriate"].includes(task.category))
        ? base + 6
        : memberId === "bed_manager" && task.source === "system"
          ? base - 3
          : memberId === "bed_manager"
            ? base + 4
            : memberId === "trusted_fy2"
              ? base - 2
              : base;
  return Math.max(20, Math.min(34, adjusted));
}

function adjustedDelegationDuration(
  state: GameState,
  task: ActiveTask,
  memberId: TeamMemberId,
): number {
  ...
  const fatigueAdjustment =
    member.fatigue >= 65 ? 5 : member.fatigue >= 45 ? 2 : 0;
  const intelAdjustment =
    task.intelLevel >= 2 ? -4 : task.intelLevel >= 1 ? -2 : 2;
  return Math.max(
    20,
    Math.min(40, base + trustAdjustment + fatigueAdjustment + intelAdjustment),
  );
}

export function isDelegationAppropriate(
  task: ActiveTask,
  memberId: TeamMemberId,
): boolean {
  ...
  if (memberId === "fy1")
    return (
      ["low", "nonsense"].includes(task.trueUrgency) ||
      (task.trueUrgency === "medium" &&
        ["routine", "inappropriate", "absurd"].includes(task.category)) ||
      (task.trueUrgency === "high" && task.category === "routine")
    );
  if (memberId === "trusted_fy2") return task.category !== "emergency";
  if (memberId === "locum_no_login")
    return (
      Boolean(task.encounterId) ||
      task.source === "system" ||
      task.trueUrgency === "medium"
    );
  if (memberId === "bed_manager")
    return (
      task.source === "system" ||
      ["routine", "inappropriate", "absurd"].includes(task.category) ||
      task.locationId === "mau"
...
function refreshOversightAt(
  ...
  const pressureRelief = wasStale
    ? 4
    : acuity.level > 55 || acuity.unresolvedRisk > 30
      ? 3
      : amount >= 8
        ? 2
        : 1;
...
function staleLocations(state: GameState): LocationId[] {
  return clinicalLocations.filter(
    (locationId) => state.minute - state.locationLastVisited[locationId] > 28,
  );
}

function applyOversightPressure(
  state: GameState,
  ...
  const rawLoss =
    Math.ceil(elapsedMinutes / 10) +
    Math.floor(stale.length / 3) +
    currentAreaFocusPenalty;
  const oversightLoss =
    state.shiftPhase === "pre_handover" ? Math.floor(rawLoss / 2) : rawLoss;
  let next = {
    ...state,
    oversight: clamp(state.oversight - oversightLoss),
    hospitalPressure: clamp(
      state.hospitalPressure +
        Math.floor(stale.length / 4) +
        (state.oversight < 35 ? 4 : 0),
    ),
  };
...
function candidateTemplates(state: GameState): TaskTemplate[] {
  const activeTemplateIds = new Set(
    state.activeTasks.map((task) => task.templateId),
  );
  const phase = shiftPhaseFor(state.minute);
  return taskTemplates
    ...
    .map((template) => {
      ...
      const acuityBonus = Math.floor(
        state.wardAcuity[template.locationId].level / 18,
      );
      const momentum = state.wardMomentum[template.locationId];
      const momentumBonus =
        Math.floor(momentum.pressure / 8) +
        (momentum.tags.includes("systemBlocked") && template.source === "system"
          ? 6
          : 0) +
        (momentum.tags.includes("quietlyUnsafe") &&
        (template.vague || template.category === "ambiguous")
          ? 6
          : 0) +
        (momentum.tags.includes("fragile") &&
        ["critical", "high"].includes(template.trueUrgency)
          ? 6
          : 0);
      const pressureBonus =
        state.hospitalPressure > 65 &&
        ["critical", "high"].includes(template.trueUrgency)
          ? 4
          : 0;
      const regSenseBonus =
        template.regSense && state.regSense > 35
          ? state.shiftPhase === "deep"
            ? 2
            : 3
          : 0;
      return {
...
function nextSpawnDelay(state: GameState, seed: number): [number, number] {
  ...
  const [roll, nextSeed] = nextRandom(seed);
  const range =
    phase === "early" ? [25, 45] : phase === "deep" ? [18, 35] : [15, 30];
  const pressureAdjustment =
    state.hospitalPressure > 70 || state.oversight < 35
      ? -8
      : state.hospitalPressure < 35 && state.oversight > 65
        ? 6
        : 0;
  const delay = Math.max(
    10,
    Math.round(range[0] + roll * (range[1] - range[0]) + pressureAdjustment),
  );
  return [delay, nextSeed];
}

export function spawnTask(state: GameState): GameState {
  const [template, rngSeed] = chooseWeighted(
    candidateTemplates(state),
    state.rngSeed,
  );
  ...
  const next = syncLegacyPagerIds(
    resolveBirdTaskStatus(
      {
        ...state,
        rngSeed,
        activeTasks: [...state.activeTasks, task],
        completedTaskIds: [
          ...new Set([...state.completedTaskIds, template.id]),
        ],
  ...
  const label = task.regSense
    ? "Reg Sense"
    : task.source === "handover"
      ? "Handover"
      : task.source === "system"
        ? "System"
        : "Bleep";
  return addLog(
    next,
    `${label}: ${task.message}`,
    task.trueUrgency === "critical" ? "bad" : "neutral",
  );
}
...
function deteriorateTask(state: GameState, task: ActiveTask): GameState {
  ...
  const severityMultiplier = Math.min(
    1.5,
    Math.max(1, state.wardAcuity[task.locationId].level / 45),
  );
  const momentum = state.wardMomentum[task.locationId];
  const momentumMultiplier =
    momentum.tags.includes("fragile") || momentum.tags.includes("quietlyUnsafe")
      ? 1.25
      : 1;
  const amplified = {
    ...task.ignored,
    patientSafety: Math.round(
      (task.ignored.patientSafety ?? 0) *
        severityMultiplier *
        momentumMultiplier,
    ),
    dangerousDelays:
      task.trueUrgency === "critical" || task.trueUrgency === "high"
        ? Math.max(1, task.ignored.dangerousDelays ?? 0)
        : task.ignored.dangerousDelays,
    datix: ["critical", "high", "medium"].includes(task.trueUrgency)
      ? Math.max(1, task.ignored.datix ?? 0)
      : task.ignored.datix,
  };
  let next = applyConsequence(state, amplified);
  next = {
    ...next,
    activeTasks: next.activeTasks.map((item) =>
      item.id === task.id
        ? {
            ...item,
            status: "deteriorated",
            deterioratedAt: next.minute,
            penaltyApplied: true,
            lastUpdatedAt: next.minute,
          }
        : item,
    ),
  ...
  next = alterWardMomentum(next, task.locationId, 12, [
    "fragile",
    "quietlyUnsafe",
  ]);
  next = remember(next, {
    deteriorations: [taskRiskLabel(task)],
    unresolvedRisks: [task.message],
    wardHotSpots: [task.locationId],
  });
  if (task.regSense && task.encounterId) {
    return addLog(
      syncLegacyPagerIds(next),
      `Reg Sense missed: ${task.message}. It is now overdue and unsafe.`,
      "bad",
    );
  }
  return addLog(
    syncLegacyPagerIds(next),
    `Delayed: ${task.message}. The situation deteriorated.`,
    "bad",
  );
}
...
export function runShiftDirector(
  state: GameState,
  elapsedMinutes: number,
): GameState {
  if (state.ended) return state;
  const highRiskTasks = state.activeTasks.filter((task) =>
    ["critical", "high"].includes(task.trueUrgency),
  ).length;
  const quietWellControlled =
    state.activeTasks.length <= 1 &&
    highRiskTasks === 0 &&
    state.oversight > 60;
  ...
  for (const locationId of clinicalLocations) {
    const activeHere = next.activeTasks.filter(
      (task) => task.locationId === locationId,
    ).length;
    if (activeHere > 0)
      next = alterWardAcuity(next, locationId, activeHere, activeHere * 2);
    if (activeHere > 0)
      next = alterWardMomentum(
        next,
        locationId,
        Math.ceil(activeHere / 2),
        activeHere >= 2 ? ["fragile"] : [],
      );
  }
  ...
  while (
    ...
    next.minute >= next.nextTaskSpawnAt
  ) {
    next = spawnTask(next);
...
function triggerLocationEncounter(state: GameState): GameState {
  ...
  const encounter = encounters.find(
    (item) =>
      ...
      !state.completedEncounterIds.includes(item.id),
  );
  ...
  return addLog(
    isBirdEncounter(encounter) ? sightBirdIfNeeded(next) : next,
    `On arrival: ${encounter.title}`,
    encounter.category === "emergency" ? "bad" : "neutral",
  );
}

export function advanceTime(state: GameState, minutes: number): GameState {
  let next = applyConsequence(state, {
    time: minutes,
    stamina: -Math.ceil(minutes / 6),
    focus: -Math.ceil(minutes / 8),
  });
  next = runShiftDirector(next, minutes);
  return checkEnding(next);
}

const treatSpawnLocations: LocationId[] = [
  "ed_resus",
  "mau",
  "respiratory",
  "cardiology",
...
function maybeTreatOnArrival(state: GameState): GameState {
  ...
  const candidates = treatTemplates.filter(
    (t) =>
      t.locationIds.length === 0 || t.locationIds.includes(state.locationId),
  );
  ...
  return addLog(
    syncLegacyPagerIds({
      ...state,
      rngSeed: nextSeed,
      activeTasks: [...state.activeTasks, treat],
    }),
    `Treat: ${treat.message}`,
    "good",
  );
...
export function moveTo(state: GameState, locationId: LocationId): GameState {
  ...
  if (
    !current.links.includes(locationId) ||
    state.ended ||
  ...
  next = refreshOversightAt(
    next,
    locationId,
    locationId === "corridor" ? 4 : 8,
  );
  next = alterWardMomentum(next, locationId, -4, [], ["quietlyUnsafe"]);
  next = addLog(
    next,
    `Moved to ${destination.name}. ${destination.quirk}`,
    friction ? "bad" : "neutral",
  );
...
function findTask(state: GameState, taskId: string): ActiveTask | undefined {
  return liveTasks(state).find(
    (task) => task.id === taskId || task.templateId === taskId,
  );
}

function removeTask(state: GameState, task: ActiveTask): GameState {
  return removeRememberedUnresolved(
    syncLegacyPagerIds({
      ...state,
...
export function clarifyTask(state: GameState, taskId: string): GameState {
  ...
  if (task.intelLevel >= 2)
    return addLog(state, `Already clear enough: ${task.message}`, "neutral");
  const cost = task.vague || task.regSense ? 4 : 2;
  const nextIntel = Math.min(
    2,
    task.intelLevel + 1,
  ) as ActiveTask["intelLevel"];
  const activeTasks = state.activeTasks.map((item) =>
    item.id === task.id
      ? {
          ...item,
          ...
          dueAt:
            item.trueUrgency === "critical"
              ? Math.max(item.dueAt, state.minute + 8)
              : item.dueAt + (task.intelLevel === 0 ? 8 : 4),
        }
      : item,
  );
  ...
  next = remember(next, {
    clarifiedRisks: [taskRiskLabel(task)],
    unresolvedRisks: ["critical", "high", "medium"].includes(task.trueUrgency)
      ? [task.message]
      : [],
  });
  next = alterWardMomentum(
...
export function markTaskForHandover(
  ...
  const activeTasks = state.activeTasks.map((item) =>
    item.id === task.id ? { ...item, markedForHandover: true } : item,
  );
  const next = remember(
    {
      ...state,
      activeTasks,
      handoverQuality: clamp(state.handoverQuality + 3),
    },
    {
      markedTasks: [taskRiskLabel(task)],
      unresolvedRisks: ["critical", "high", "medium"].includes(task.trueUrgency)
        ? [task.message]
        : [],
    },
  ...
  return addLog(
    syncLegacyPagerIds(next),
    `Marked for handover: ${task.message}`,
    "good",
  );
}

function escalationTimingForTask(
  state: GameState,
  task: ActiveTask,
): "early" | "late" | "premature" {
...
function escalationConsequence(
  target: EscalationTarget,
  timing: "early" | "late" | "premature",
  intelLevel: ActiveTask["intelLevel"] = 1,
): Consequence {
  if (timing === "premature")
    return { time: 3, focus: -4, reputation: -4, score: -5 };
  if (timing === "late")
    return {
      time: 5,
      patientSafety: 3,
      ...
      handoverQuality: 3,
      consultantEscalations: 1,
      score: 20,
    };
  const intelBonus = intelLevel >= 2 ? 5 : 0;
  return {
    time: target === "ICU" ? 5 : 4,
    patientSafety: 5 + intelBonus,
    reputation: 2,
    clinicalConfidence: 4,
    handoverQuality: 4,
    consultantEscalations: target === "consultant" || target === "ICU" ? 1 : 0,
    score: 35 + intelBonus,
  };
}
...
export function escalateTask(state: GameState, taskId: string): GameState {
  ...
  const record = {
    id: `${task.id}:${state.minute}:${target}`,
    minute: state.minute,
    target,
  ...
  let next = applyConsequence(
    state,
    escalationConsequence(target, timing, task.intelLevel),
  );
  next = {
    ...next,
    ...
    activeTasks: next.activeTasks.map((item) =>
      item.id === task.id
        ? {
            ...
            markedForHandover: true,
            lastUpdatedAt: next.minute,
            dueAt:
              timing === "late"
                ? Math.max(next.minute + 6, item.dueAt)
                : item.dueAt + 8,
          }
        : item,
    ),
    hospitalPressure: clamp(
      next.hospitalPressure + (timing === "premature" ? 2 : -4),
    ),
  };
  next = runShiftDirector(
    next,
    escalationConsequence(target, timing, task.intelLevel).time ?? 0,
  );
  next = remember(next, {
    escalations: [`${target} for ${task.message}`],
    markedTasks: [taskRiskLabel(task)],
    unresolvedRisks: [task.message],
  });
  next = alterWardMomentum(
    next,
    task.locationId,
    timing === "premature" ? 1 : -5,
    [],
    timing === "early" ? ["fragile"] : [],
  );
  const timingText =
    timing === "early"
  ...
  return addLog(
    syncLegacyPagerIds(next),
    `Escalated to ${target}: ${task.message}. Timing: ${timingText}.`,
    timing === "premature" ? "neutral" : "good",
  );
}
...
export function markEncounterForHandover(state: GameState): GameState {
  ...
  const next = remember(
    { ...state, handoverQuality: clamp(state.handoverQuality + 4) },
    {
      markedTasks: [
        `${encounter.title} (${locations.find((location) => location.id === encounter.locationId)?.name ?? encounter.locationId})`,
      ],
      unresolvedRisks: [encounter.title],
    },
  );
  return addLog(
    next,
    `Marked active encounter for handover: ${encounter.title}`,
    "good",
  );
}
...
export function escalateEncounter(state: GameState): GameState {
  ...
  if (!encounter || state.ended) return state;
  const target = escalationTargetFor({
    ...encounter,
    source: "pager" as const,
    trueUrgency: encounter.category === "emergency" ? "critical" : "high",
    encounterId: encounter.id,
  });
  const record = {
    id: `encounter:${encounter.id}:${state.minute}:${target}`,
    minute: state.minute,
    target,
  ...
  next = runShiftDirector(
    next,
    escalationConsequence(target, "early").time ?? 0,
  );
  next = remember(next, {
    escalations: [`${target} for ${encounter.title}`],
    markedTasks: [encounter.title],
    unresolvedRisks: [encounter.title],
  });
...
export function respondToPager(state: GameState, taskId: string): GameState {
  ...
  let next = removeTask(
    resolveBirdTaskStatus(
      { ...state, locationId: task.locationId },
      task,
      "sighted",
    ),
    task,
  ...
  if (task.encounterId) {
    next = {
      ...next,
      activeEncounterId: task.encounterId,
      activeEncounterStepId: firstEncounterStepId(task.encounterId),
    };
  ...
  next = {
    ...next,
    hospitalPressure: clamp(
      next.hospitalPressure - pressureReliefForTask(task),
    ),
  };
  ...
  const activeTasks = state.activeTasks.map((item) =>
    item.id === task.id
      ? {
          ...
          lastUpdatedAt: state.minute,
          dueAt: Math.max(state.minute + 6, item.dueAt - 4 - vaguePenalty),
        }
      : item,
  );
  let next = advanceTime(
    {
      ...state,
      activeTasks,
      deferredPagerIds: [...new Set([...state.deferredPagerIds, task.id])],
      hospitalPressure: clamp(
        state.hospitalPressure + 4 + (vaguePenalty ? 2 : 0),
      ),
    },
    3,
  );
  next = alterWardAcuity(
    next,
    task.locationId,
    3 + (vaguePenalty ? 2 : 0),
    4 + vaguePenalty,
...
export function ignorePager(state: GameState, taskId: string): GameState {
  ...
  next = alterWardAcuity(
    next,
    task.locationId,
    ["critical", "high"].includes(task.trueUrgency) ? 10 : 3,
    task.vague ? 10 : 5,
  );
  next = alterWardMomentum(
    next,
    task.locationId,
    ["critical", "high"].includes(task.trueUrgency) ? 12 : 5,
    momentumTagsForTask(task),
  );
  next = {
    ...next,
    hospitalPressure: clamp(
      next.hospitalPressure + (task.trueUrgency === "nonsense" ? -2 : 6),
    ),
  };
  if (["critical", "high", "medium"].includes(task.trueUrgency))
    next = remember(next, {
      unresolvedRisks: [task.message],
      deteriorations: [taskRiskLabel(task)],
      wardHotSpots: [task.locationId],
...
export function delegatePager(
  state: GameState,
  taskId: string,
  memberId: TeamMemberId = "trusted_fy2",
): GameState {
  const task = findTask(state, taskId);
  if (!task || state.ended) return state;
  const member = state.team.find((item) => item.id === memberId);
  if (!member || member.busyUntil > state.minute) {
    return addLog(
      state,
      `${member?.name ?? "Team member"} is not available for delegation.`,
      "bad",
    );
  }
  ...
  next = {
    ...next,
    team: next.team.map((item) =>
      item.id === memberId
        ? {
            ...item,
            busyUntil: next.minute + duration,
            trust: clamp(
              item.trust + (appropriate ? (item.id === "fy1" ? 6 : 4) : -8),
            ),
            fatigue: clamp(
              item.fatigue + Math.ceil(duration / 6) + (appropriate ? 0 : 6),
            ),
    ...
    hospitalPressure: clamp(
      next.hospitalPressure +
        (appropriate
          ? -Math.max(3, Math.floor(pressureReliefForTask(task) / 2))
          : 5),
    ),
  };
  if (appropriate || !["critical", "high"].includes(task.trueUrgency)) {
    ...
    next = resolveBirdTaskStatus(
      next,
      task,
      appropriate ? "contained" : "loose",
    );
  } else {
    next = {
      ...next,
      activeTasks: next.activeTasks.map((item) =>
        item.id === task.id
          ? {
              ...
              deferred: true,
              lastUpdatedAt: next.minute,
              dueAt: Math.max(next.minute + 4, item.dueAt - 10),
            }
          : item,
      ),
    };
  ...
  next = alterWardMomentum(
    ...
    task.locationId,
    appropriate ? -3 : 7,
    appropriate ? [] : momentumTagsForTask(task),
    appropriate ? ["systemBlocked"] : [],
  );
  next = remember(next, {
    delegatedJobs: [`${task.message} -> ${member.name}`],
    unresolvedRisks: appropriate ? [] : [task.message],
  });
  return addLog(
    next,
    appropriate
      ? `Delegated to ${member.name} for ${duration}m: ${task.message}`
      : `Poor delegation to ${member.name}: ${task.message}`,
    appropriate ? "good" : "bad",
  );
}
...
function hashText(value: string): number {
  return value
    .split("")
    .reduce((sum, char) => (sum * 31 + char.charCodeAt(0)) >>> 0, 2166136261);
}
...
const syntheticStepEncounterIds = new Set([
  "sepsis_hypotension",
  "copd_hypercapnia",
  "nstemi",
  "aki_hyperkalaemia",
...
export function firstEncounterStepId(encounterId: string): string | undefined {
  ...
  return (
    encounter?.steps?.[0]?.id ??
    (syntheticStepEncounterIds.has(encounterId) ? "assessment" : undefined)
  );
}
...
export function currentEncounterStep(encounter?: Encounter, stepId?: string) {
  ...
  return (
    encounter.steps.find((step) => step.id === stepId) ?? encounter.steps[0]
  );
}
      ...
      choices: [
        {
          id: "assessment_best",
          label:
            "Do an ABCDE review, identify immediate risks, and ask for the right help early",
          detail:
            "You build a working problem representation before committing to treatment.",
          feedback:
            "The situation is clearer, and the ward team understand what you are worried about.",
        ...
        },
        {
          ...
          detail:
            "Some useful context, but the patient is still deteriorating in real time.",
        ...
        },
        {
          ...
          detail:
            "This underestimates the risk and leaves the team without a plan.",
...
export function orderedEncounterChoices(
  state: GameState,
  encounter: Encounter,
): EncounterChoice[] {
  ...
  const choices =
    encounter.id === "consultant_grilling"
      ? baseChoices.map((choice) => {
          ...
          if (choice.id === "best" && memoryScore >= 65) {
            modified = {
              ...modified,
              label:
                "Give a structured risk handover backed by the things you flagged overnight",
              consequence: {
                ...modified.consequence,
                handoverQuality:
                  (modified.consequence.handoverQuality ?? 0) + 6,
                reputation: (modified.consequence.reputation ?? 0) + 3,
                score: (modified.consequence.score ?? 0) + 80,
              },
            };
          }
          if (choice.id === "unsafe" && memoryScore < 45) {
            modified = {
              ...modified,
              label:
                "Try to imply the night was quieter than the memory trail suggests",
              consequence: {
                ...modified.consequence,
                handoverQuality:
                  (modified.consequence.handoverQuality ?? 0) - 6,
          ...
          if (state.birdStatus === "contained" && choice.id === "best") {
            modified = {
              ...
              consequence: {
                ...modified.consequence,
                score: (modified.consequence.score ?? 0) + 30,
              },
            };
          }
          if (state.birdStatus === "loose" && choice.id === "unsafe") {
            modified = {
              ...
              consequence: {
                ...modified.consequence,
                reputation: (modified.consequence.reputation ?? 0) - 3,
                score: (modified.consequence.score ?? 0) - 20,
              },
            };
          }
          return modified;
  ...
  return shuffleChoices(
    choices,
    state.rngSeed ^ hashText(`${encounter.id}:${step?.id ?? "base"}`),
  );
}
...
export function chooseEncounterOption(
  ...
  if (choice.unlockItem && !next.items.includes(choice.unlockItem))
    next = { ...next, items: [...next.items, choice.unlockItem] };
  if (choice.nextStepId) {
    ...
    return addLog(
      { ...syncLegacyPagerIds(next), activeEncounterStepId: choice.nextStepId },
      `${encounter.title}: ${choice.feedback}`,
      choice.unsafe ? "bad" : "neutral",
    );
  }
  next = {
    ...next,
    ...
    activeEncounterStepId: undefined,
    handoverGrillingDone:
      encounter.id === "consultant_grilling" ? true : next.handoverGrillingDone,
    completedEncounterIds: [
      ...new Set([...next.completedEncounterIds, encounter.id]),
    ],
    resolvedPagerIds: [...next.resolvedPagerIds, `encounter:${encounter.id}`],
    hospitalPressure: clamp(
      next.hospitalPressure + pressureDeltaForEncounterChoice(choice),
    ),
  };
  ...
  next = alterWardMomentum(
    ...
    encounter.locationId,
    choice.unsafe ? 9 : -7,
    choice.unsafe ? ["fragile"] : [],
    choice.unsafe ? [] : ["quietlyUnsafe"],
  );
  next = resolveBirdEncounterStatus(next, encounter, choice);
  if (!choice.unsafe && choice.id === "best")
    next = maybeAwardEncounterResource(next, encounter);
  next = remember(
    next,
    choice.unsafe
      ? {
          unresolvedRisks: [encounter.title],
          wardHotSpots: [encounter.locationId],
        }
      : { resolvedEncounters: [encounter.title] },
  );
  next = addLog(
    syncLegacyPagerIds(next),
    `${encounter.title}: ${choice.feedback}`,
    choice.unsafe ? "bad" : choice.id === "best" ? "good" : "neutral",
  );
  if (encounter.id === "consultant_grilling") {
    return {
      ...
      minute: SHIFT_LENGTH,
      ended: true,
      endingReason: "Morning handover survived, including consultant grilling.",
    };
  }
  return next;
}
...
export function takeBreak(state: GameState): GameState {
  ...
  let next = applyConsequence(state, {
    ...
    stamina: 16,
    focus: 10,
    caffeine: caffeineHit,
    breaksTaken: 1,
    score: state.activeTasks.length ? -10 : 20,
  });
  next = {
    ...next,
    oversight: clamp(next.oversight - (state.activeTasks.length ? 8 : 4)),
    hospitalPressure: clamp(
      next.hospitalPressure - (state.activeTasks.length ? 0 : 8),
    ),
  };
  next = runShiftDirector(
    next,
    9 + Math.min(8, state.activeTasks.length * 2 + state.breaksTaken),
  );
  const warning = state.activeTasks.length
    ? "Hospital pressure rises while you rest; the bleep finds you anyway."
    : "You take nine protected-ish minutes in the mess.";
...
export function brewCoffee(state: GameState): GameState {
  ...
  let next = applyConsequence(state, {
    ...
    focus: 5,
    stamina: 2,
    score: state.activeTasks.length ? 0 : 5,
  });
  next = runShiftDirector(
    {
      ...next,
      hospitalPressure: clamp(
        next.hospitalPressure + (state.activeTasks.length ? 2 : 0),
      ),
    },
    3,
  );
  return addLog(
    ...
    state.activeTasks.length
      ? "You make coffee while the bleep stack watches you make choices."
      : "You make coffee. It tastes like survival with undertones of kettle.",
    "good",
  );
}
...
export function findSnack(state: GameState): GameState {
  if (
    state.ended ||
    !["mess", "corridor", "lifts", "pharmacy"].includes(state.locationId)
  )
    return state;
  const pharmacyBonus = state.locationId === "pharmacy" ? 3 : 0;
  let next = applyConsequence(state, {
    time: 4,
    stamina: 12 + pharmacyBonus,
    focus: 3,
    caffeine: 2,
    score: state.activeTasks.length ? 0 : 5,
  });
  next = runShiftDirector(
    {
      ...next,
      hospitalPressure: clamp(
        next.hospitalPressure + (state.activeTasks.length ? 1 : -1),
      ),
    },
    4,
  );
  const source =
    state.locationId === "pharmacy"
      ? "The pharmacy hatch produces a medically unlicensed biscuit."
      : state.locationId === "mess"
        ? "You find a snack in the mess cupboard and choose not to inspect the expiry date."
        : "The vending machine accepts your coins after a brief ethical debate.";
  return addLog(syncLegacyPagerIds(next), source, "good");
}

function changeResourceCharges(
  ...
  return resources.map((resource) =>
    resource.id === resourceId
      ? { ...resource, charges: Math.max(0, resource.charges + delta) }
      : resource,
  );
}
...
function maybeAwardResource(state: GameState, task: ActiveTask): GameState {
  const award: ResourceItemId | undefined =
    task.source === "system" && task.locationId === "pharmacy"
      ? "snack"
      : task.locationId === "radiology"
        ? "radiology_persuasion"
        : task.category === "routine" && task.trueUrgency === "medium"
          ? "cannula_kit"
          : task.category === "absurd"
            ? "coffee"
            : undefined;
  if (!award) return state;
  return {
    ...state,
    resources: changeResourceCharges(state.resources, award, 1),
  };
}

function maybeAwardEncounterResource(
  state: GameState,
  encounter: Encounter,
): GameState {
  const award: ResourceItemId | undefined =
    encounter.locationId === "radiology"
      ? "radiology_persuasion"
      : ["copd_hypercapnia", "dka", "aki_hyperkalaemia"].includes(encounter.id)
        ? "abg_kit"
        : undefined;
  if (!award) return state;
  return {
...
export function consumeResource(
  ...
  if (!resource || resource.charges <= 0 || state.ended)
    return addLog(
      state,
      `${resource?.label ?? "Resource"} is not available.`,
      "bad",
    );
  if (resource.usableWhen === "encounter" && !state.activeEncounterId)
    return addLog(
      state,
      `${resource.label} is only useful while a patient is in front of you.`,
      "neutral",
    );
  if (resource.usableWhen === "task" && state.activeTasks.length === 0)
    return addLog(
      state,
      `${resource.label} needs a live task to make a difference.`,
      "neutral",
    );
  ...
  next = {
    ...next,
    resources: changeResourceCharges(next.resources, resourceId, -1),
  };
  if ((effects[resourceId].time ?? 0) > 0)
    next = runShiftDirector(next, effects[resourceId].time ?? 0);
  return addLog(
    syncLegacyPagerIds(next),
    `Used resource: ${resource.label}.`,
    "good",
  );
}
...
export function checkEnding(state: GameState): GameState {
  if (state.ended) return state;
  if (
    state.minute >= SHIFT_LENGTH &&
    !state.handoverGrillingDone &&
    !state.activeEncounterId
  ) {
    return addLog(
      {
        ...state,
        minute: SHIFT_LENGTH,
        activeEncounterId: "consultant_grilling",
        activeEncounterStepId: firstEncounterStepId("consultant_grilling"),
        locationId: "mau",
      },
      "Surprise final task: consultant grilling at handover.",
      "bad",
    );
  }
  if (
    state.minute >= SHIFT_LENGTH &&
    state.activeEncounterId === "consultant_grilling"
  )
    return { ...state, minute: SHIFT_LENGTH };
  if (state.minute >= SHIFT_LENGTH)
    return {
      ...state,
      minute: SHIFT_LENGTH,
      ended: true,
      endingReason: "Morning handover reached.",
    };
  if (state.patientSafety <= 0)
    return {
      ...state,
      ended: true,
      endingReason: "Patient safety collapsed before handover.",
    };
  if (state.stamina <= 0 || state.focus <= 0)
    return {
      ...state,
      ended: true,
      endingReason: "Registrar collapse: fatigue and cognitive load won.",
    };
  if (state.oversight <= 0 && state.hospitalPressure > 80)
    return {
      ...state,
      ended: true,
      endingReason: "Oversight collapsed: the hospital ran away from you.",
    };
  if (
    state.hospitalPressure >= 100 &&
    ...
    return {
      ...state,
      ended: true,
      endingReason: "Hospital pressure overwhelmed the night team.",
    };
  return state;
}
...
export function endingRank(state: GameState): string {
  const total =
    state.score +
    state.patientSafety * 6 +
    state.reputation * 3 +
    handoverMemoryScore(state) * 3 +
    state.oversight * 2 -
    state.dangerousDelays * 90 -
    state.datix * 60 -
    state.hospitalPressure * 2;
